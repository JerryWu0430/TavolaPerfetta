...
def get_supplier_stats(supplier_id: int, restaurant_id: int, db: Session) -> dict:
...
def list_suppliers(
    ...
    query = db.query(Supplier).filter(Supplier.restaurant_id == user.restaurant_id)
    if category:
    ...
    for supplier in items:
        stats = get_supplier_stats(supplier.id, user.restaurant_id, db)
...
def get_supplier(
    ...
    supplier = db.query(Supplier).filter(
        ...
        Supplier.restaurant_id == user.restaurant_id,
    ).first()
    if not supplier:
        raise HTTPException(status_code=404, detail="Supplier not found")

    stats = get_supplier_stats(supplier_id, user.restaurant_id, db)
...
def create_supplier(
    ...
    supplier = Supplier(**data.model_dump(), restaurant_id=user.restaurant_id)
    db.add(supplier)
    db.commit()
    db.refresh(supplier)
    return supplier
